...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--logpath",
        type=log_file_valid_accessible,
        nargs="?",
        default=None,
        help="Application log file to save logger output",
    )

    parser.add_argument("--log_max_bytes", type=int, nargs="?", default=None, help="Max log file size (bytes)")

    parser.add_argument("--log_backup_count", type=int, nargs="?", default=None, help="Max log file count")

    parser.add_argument(
        "--config",
        type=config_file_exists_accessible,
        nargs="?",
    ...
    parser.add_argument(
        "--app", type=available_apps, nargs="?", default=None, required=True, help="Application name to run"
    )

    parser.add_argument(
        "--params", dest="parameters", action=StoreDictKeyPair, nargs="+", metavar="KEY=VAL", default={}
    )

    parser.add_argument(
        "--ignore-proxy", default=False, const=True, nargs="?", help="Ignore proxy configuration and environment"
    )

    parser.add_argument(
        "--data_dir",
        type=data_dir_exists_accessible,
        nargs="?",
        default=None,
